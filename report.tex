...
\begin{figure}[h]
  \centering
  % QLoRA run – stable, low‑loss convergence
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{QLoRATrainer_WandB_25s_a75.pdf}
    \caption*{(a) \textbf{QLoRATrainer}: smooth loss trajectory and bounded gradients.}
  \end{minipage}\hfill
  % Baseline solver – oscillations and spikes
  \begin{minipage}{0.48\linewidth}
    \centering
    \includegraphics[width=\linewidth]{GSM8KSolver_WandB_100s.pdf}
    \caption*{(b) \textbf{GSM8KSolver}: higher final loss and two pronounced gradient‐norm spikes.}
  \end{minipage}

  \vspace{4pt}
  \caption{Training‑dynamics comparison of two representative configurations explored in the hyper‑parameter grid.  
           The QLoRA setup (rank 16, $\alpha$ 32, lr $1\!\times\!10^{-4}$) reaches a lower loss floor and keeps gradients below 0.035, 
           while the baseline solver shows oscillations and spikes up to 0.65, motivating the stability tweaks described above.}
